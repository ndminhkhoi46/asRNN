...
class Orthogonal(Parametrization):
    ...
    def forward(self, input):
        return input.mm(self.B.t())
        
    def inverse_forward(self, input):
        return input.mm(self.B)
...
class modrelu(nn.Module):
    ...
    def reset_parameters(self):
        self.b.data = nn.init.uniform_(self.b.data, -0.01, 0.01)
...
class OrthogonalRNN(nn.Module):
    def __init__(self, input_size, hidden_size, nonlinearity, has_bias, initializer_skew, mode, param, input_initializer = nn.init.kaiming_normal_):
